...
CREATE INDEX idx_trade_news_active ON public.trade_news USING btree (is_active, expires_at);
...
create trigger update_trade_news_updated_at before
update
    on
    public.trade_news for each row execute function update_updated_at_column();
