...
-- public.trade_news_cache definition

-- Drop table

-- DROP TABLE public.trade_news_cache;

CREATE TABLE public.trade_news_cache (
	id int8 GENERATED ALWAYS AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1 NO CYCLE) NOT NULL,
	title varchar(500) NOT NULL,
	summary text NULL,
	source_name varchar(200) NOT NULL,
	...
	is_active bool DEFAULT true NOT NULL,
	created_at timestamp DEFAULT CURRENT_TIMESTAMP NULL,
	CONSTRAINT trade_news_cache_pkey PRIMARY KEY (id),
	CONSTRAINT trade_news_cache_source_url_fetched_at_key UNIQUE (source_url, fetched_at)
);
CREATE INDEX idx_trade_news_active ON public.trade_news_cache USING btree (is_active, expires_at);
CREATE INDEX idx_trade_news_category ON public.trade_news_cache USING btree (category);
CREATE INDEX idx_trade_news_priority ON public.trade_news_cache USING btree (priority, published_at DESC);
CREATE INDEX idx_trade_news_published ON public.trade_news_cache USING btree (published_at DESC);

-- Table Triggers

create trigger update_trade_news_cache_updated_at before
update
    on
    public.trade_news_cache for each row execute function update_updated_at_column();
